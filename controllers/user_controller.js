  ...
  try {
    ...
    if (!req.session.isLoggedInUser) {
      ...
      const application = await Application.findOne({
        "applicant_details._id": req.user._id,
      });
  ...
  try {
    ...
    if (search) {
      ...
      const application = await Application.findByIdAndUpdate(
        { _id: search._id },
        { $set: { job_posts: updated_job_posts, documents: documents } }
      );
      if (application) {
...
exports.getApplication = async (req, res) => {
  ...
  try {
    ...
    const application_status = await ApplicationStatus.find({
  ...
  try {
    ...
    const updateFields = {
      $set: {
        ...
        email: email,
        mobile: mobile,
        "profile.work_experience.company_name": company_name,
        "profile.work_experience.job_title": job_title,
        "profile.work_experience.duration": duration,
        "profile.education.degree": degree,
        "profile.education.institute_name": institute_name,
        "profile.education.field": field,
        "profile.education.graduation": graduation,
      },
    };
    const update_profile = await Applicant.findByIdAndUpdate(
      { _id: userId },
      updateFields
    );
    if (update_profile) {
...
exports.postResetPassword = async (req, res) => {
  ...
  const filePath = path.join(
    __dirname,
    "../views/user/reset_password_mail.ejs"
  );
  try {
...
exports.postResetPasswordConfirm = async (req, res) => {
  ...
  try {
    let resetUser = await Applicant.findOne({
