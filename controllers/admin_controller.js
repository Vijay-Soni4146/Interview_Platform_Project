...
exports.postApproved = async (req, res) => {
  ...
  try {
    ...
    const application_delete = await Application.findOne({
      _id: applicationId,
    });
    const updated_application_jobPost = application_delete.job_posts.filter(
      (jobPost) => {
        return jobPost._id.toString() !== jobPostId.toString();
      }
    );

    const updated_application_document = application_delete.documents.filter(
      (document) => {
        return document._id.toString() !== documentId.toString();
      }
    );

    const updated_application = await Application.findByIdAndUpdate(
      { _id: applicationId },
      {
        $set: {
          job_posts: updated_application_jobPost,
          documents: updated_application_document,
        },
      }
    );
...
exports.getApproved = async (req, res) => {
  try {
    const approvedApplication = await ApplicationStatus.find({
      status: "approved",
...
exports.getRejected = async (req, res) => {
  try {
    const rejectedApplication = await ApplicationStatus.find({
      status: "rejected",
    }).populate("applicantId");
...
exports.getApprovedDetailApplicationEvaluation = async (req, res) => {
  ...
  // console.log(postId);
  try {
    const applications = await ApplicationStatus.findOne({
      applicationId: applicationId,
      "jobPostId._id": new mongoose.Types.ObjectId(postId),
    }).populate("applicantId applicationId");
    console.log(applications);
...
exports.postRescheduleApprovedEvaluation = async (req, res) => {
  ...
  try {
    const status = await ApplicationStatus.findOneAndUpdate(
      {
        applicationId: new mongoose.Types.ObjectId(applicationId),
        "jobPostId._id": new mongoose.Types.ObjectId(jobPostId),
      },
      { $set: { date_scheduled: dateTime } }
    );
    // console.log(status);
    notifier.notify({
      title: "Rescheduled",
...
exports.postResetPasswordAdmin = async (req, res) => {
  ...
  const filePath = path.join(
    __dirname,
    "../views/admin/reset_password_mail_admin.ejs"
  );
  try {
...
exports.postResetPasswordConfirmAdmin = async (req, res) => {
  ...
  try {
    let resetAdmin = await Recruiter.findOne({
